...
namespace DAL
{
    public class DBHpelp
    {
        ...
        public static bool Open()  //创建数据库连接
        {
            //如果对象为空，则创建新的连接对象
            if (conn == null)
                conn = new SqlConnection(connStr);

            //打开数据库连接，如果成功则返回true，如果发生异常则返回false
            try
            {
                conn.Open();
                return true;
        ...
        public static SqlDataReader ExecuteReader(string strSql, SqlParameter[] p = null)  //查询
        {
        ...
        public static int ExecuteNonQuert(string strSql, SqlParameter[] p = null)  //增删改查
        {
            int count = 0;
            try
            {
